...
//! Gate: criteria_merged
//! JSON passthrough gate with path verification.
//! Validates against criteria-merged schema and verifies all paths exist.
...
fn gate_validate(input: &str) -> Result<String, NornirError> {
    let result = CRITERIA_MERGED.validate(input)?;
    if !result.valid {
        return Err(error_core::SchemaError::ValidationFailed(result.message).into());
    }
    path_verify::verify_paths(CRITERIA_MERGED.schema_json(), input)?;
    Ok(input.to_string())
}
...
#[pymodule]
fn gate_criteria_merged(m: &Bound<'_, PyModule>) -> PyResult<()> {
