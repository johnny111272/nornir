...
//! Gate: guardrails_reduced
//! JSON passthrough gate with path verification.
//! Validates against guardrails-reduced schema and verifies all paths exist.
...
fn gate_validate(input: &str) -> Result<String, NornirError> {
    let result = GUARDRAILS_REDUCED.validate(input)?;
    if !result.valid {
        return Err(error_core::SchemaError::ValidationFailed(result.message).into());
    }
    path_verify::verify_paths(GUARDRAILS_REDUCED.schema_json(), input)?;
    Ok(input.to_string())
}
...
#[pymodule]
fn gate_guardrails_reduced(m: &Bound<'_, PyModule>) -> PyResult<()> {
