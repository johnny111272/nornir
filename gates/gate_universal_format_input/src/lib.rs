//! Gate: gate_universal_format_input
//! Input gate: reads TOML from disk, validates against universal-format schema, returns JSON.
...
fn validate(py: Python<'_>, path: &str) -> PyResult<PyObject> {
    match read_and_validate(&UNIVERSAL_FORMAT, path, true) {
        Ok(json) => {
...
fn is_valid(_py: Python<'_>, path: &str) -> PyResult<bool> {
    Ok(read_and_validate(&UNIVERSAL_FORMAT, path, true).is_ok())
}
...
#[pymodule]
fn gate_universal_format_input(m: &Bound<'_, PyModule>) -> PyResult<()> {
