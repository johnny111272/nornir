//! Gate: gate_execution_merged_output
//! Output gate: validates JSON against execution-merged schema, writes TOML to disk.
...
fn validate(py: Python<'_>, data: &str, path: &str) -> PyResult<PyObject> {
    match validate_and_write(&EXECUTION_MERGED, data, path, true) {
        Ok(()) => {
...
fn is_valid(_py: Python<'_>, data: &str) -> PyResult<bool> {
    Ok(EXECUTION_MERGED.is_valid(data).unwrap_or(false))
}
...
#[pymodule]
fn gate_execution_merged_output(m: &Bound<'_, PyModule>) -> PyResult<()> {
