...
//! Gate: execution_merged
//! JSON passthrough gate with path verification.
//! Validates against execution-merged schema and verifies all paths exist.
...
fn gate_validate(input: &str) -> Result<String, NornirError> {
    let result = EXECUTION_MERGED.validate(input)?;
    if !result.valid {
        return Err(error_core::SchemaError::ValidationFailed(result.message).into());
    }
    path_verify::verify_paths(EXECUTION_MERGED.schema_json(), input)?;
    Ok(input.to_string())
}
...
#[pymodule]
fn gate_execution_merged(m: &Bound<'_, PyModule>) -> PyResult<()> {
