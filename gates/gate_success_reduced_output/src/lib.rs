//! Gate: gate_success_reduced_output
//! Output gate: validates JSON against success-reduced schema, writes TOML to disk.
...
fn validate(py: Python<'_>, data: &str, path: &str) -> PyResult<PyObject> {
    match validate_and_write(&SUCCESS_REDUCED, data, path, true) {
        Ok(()) => {
...
fn is_valid(_py: Python<'_>, data: &str) -> PyResult<bool> {
    Ok(SUCCESS_REDUCED.is_valid(data).unwrap_or(false))
}
...
#[pymodule]
fn gate_success_reduced_output(m: &Bound<'_, PyModule>) -> PyResult<()> {
