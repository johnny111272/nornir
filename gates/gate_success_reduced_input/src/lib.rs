//! Gate: gate_success_reduced_input
//! Input gate: reads TOML from disk, validates against success-reduced schema, returns JSON.
...
fn validate(py: Python<'_>, path: &str) -> PyResult<PyObject> {
    match read_and_validate(&SUCCESS_REDUCED, path, true) {
        Ok(json) => {
...
fn is_valid(_py: Python<'_>, path: &str) -> PyResult<bool> {
    Ok(read_and_validate(&SUCCESS_REDUCED, path, true).is_ok())
}
...
#[pymodule]
fn gate_success_reduced_input(m: &Bound<'_, PyModule>) -> PyResult<()> {
