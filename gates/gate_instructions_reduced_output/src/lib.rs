//! Gate: gate_instructions_reduced_output
//! Output gate: validates JSON against instructions-reduced schema, writes TOML to disk.
...
fn validate(py: Python<'_>, data: &str, path: &str) -> PyResult<PyObject> {
    match validate_and_write(&INSTRUCTIONS_REDUCED, data, path, true) {
        Ok(()) => {
...
fn is_valid(_py: Python<'_>, data: &str) -> PyResult<bool> {
    Ok(INSTRUCTIONS_REDUCED.is_valid(data).unwrap_or(false))
}
...
#[pymodule]
fn gate_instructions_reduced_output(m: &Bound<'_, PyModule>) -> PyResult<()> {
