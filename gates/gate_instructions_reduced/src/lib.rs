...
//! Gate: instructions_reduced
//! JSON passthrough gate with path verification.
//! Validates against instructions-reduced schema and verifies all paths exist.
...
fn gate_validate(input: &str) -> Result<String, NornirError> {
    let result = INSTRUCTIONS_REDUCED.validate(input)?;
    if !result.valid {
        return Err(error_core::SchemaError::ValidationFailed(result.message).into());
    }
    path_verify::verify_paths(INSTRUCTIONS_REDUCED.schema_json(), input)?;
    Ok(input.to_string())
}
...
#[pymodule]
fn gate_instructions_reduced(m: &Bound<'_, PyModule>) -> PyResult<()> {
