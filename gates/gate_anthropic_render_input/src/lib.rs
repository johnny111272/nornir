//! Gate: gate_anthropic_render_input
//! Input gate: reads TOML from disk, validates against anthropic-render schema, returns JSON.
...
fn validate(py: Python<'_>, path: &str) -> PyResult<PyObject> {
    match read_and_validate(&ANTHROPIC_RENDER, path, true) {
        Ok(json) => {
...
fn is_valid(_py: Python<'_>, path: &str) -> PyResult<bool> {
    Ok(read_and_validate(&ANTHROPIC_RENDER, path, true).is_ok())
}

#[pyfunction]
fn schema_name() -> &'static str {
    ANTHROPIC_RENDER.schema_name()
}

#[pymodule]
fn gate_anthropic_render_input(m: &Bound<'_, PyModule>) -> PyResult<()> {
