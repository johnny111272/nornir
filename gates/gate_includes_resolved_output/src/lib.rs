//! Gate: gate_includes_resolved_output
//! Output gate: validates JSON against includes-resolved schema, writes TOML to disk.
...
fn validate(py: Python<'_>, data: &str, path: &str) -> PyResult<PyObject> {
    match validate_and_write(&INCLUDES_RESOLVED, data, path, true) {
        Ok(()) => {
...
fn is_valid(_py: Python<'_>, data: &str) -> PyResult<bool> {
    Ok(INCLUDES_RESOLVED.is_valid(data).unwrap_or(false))
}
...
#[pymodule]
fn gate_includes_resolved_output(m: &Bound<'_, PyModule>) -> PyResult<()> {
