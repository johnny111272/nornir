//! Gate: gate_universal_render_input
//! Input gate: reads TOML from disk, validates against universal-render schema, returns JSON.
...
fn validate(py: Python<'_>, path: &str) -> PyResult<PyObject> {
    match read_and_validate(&UNIVERSAL_RENDER, path, true) {
        Ok(json) => {
...
fn is_valid(_py: Python<'_>, path: &str) -> PyResult<bool> {
    Ok(read_and_validate(&UNIVERSAL_RENDER, path, true).is_ok())
}
