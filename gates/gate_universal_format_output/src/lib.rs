//! Gate: gate_universal_format_output
//! Output gate: validates JSON against universal-format schema, writes TOML to disk.
...
fn validate(py: Python<'_>, data: &str, path: &str) -> PyResult<PyObject> {
    match validate_and_write(&UNIVERSAL_FORMAT, data, path, true) {
        Ok(()) => {
...
fn is_valid(_py: Python<'_>, data: &str) -> PyResult<bool> {
    Ok(UNIVERSAL_FORMAT.is_valid(data).unwrap_or(false))
}
