//! Gate: gate_guardrails_reduced_input
//! Input gate: reads TOML from disk, validates against guardrails-reduced schema, returns JSON.

use pyo3::prelude::*;
use pyo3::types::PyDict;

use gate_io::read_and_validate;
use schemas_embedded::GUARDRAILS_REDUCED;

#[pyfunction]
fn validate(py: Python<'_>, path: &str) -> PyResult<PyObject> {
    match read_and_validate(&GUARDRAILS_REDUCED, path, true) {
        Ok(json) => {
...
fn is_valid(_py: Python<'_>, path: &str) -> PyResult<bool> {
    Ok(read_and_validate(&GUARDRAILS_REDUCED, path, true).is_ok())
}

#[pyfunction]
fn schema_name() -> &'static str {
    GUARDRAILS_REDUCED.schema_name()
}

#[pymodule]
fn gate_guardrails_reduced_input(m: &Bound<'_, PyModule>) -> PyResult<()> {
