//! Gate: gate_galdr_style_input
//! Input gate: reads TOML from disk, validates against galdr-style schema, returns JSON.

use pyo3::prelude::*;
use pyo3::types::PyDict;

use gate_io::read_and_validate;
use schemas_embedded::GALDR_STYLE;

#[pyfunction]
fn validate(py: Python<'_>, path: &str) -> PyResult<PyObject> {
    match read_and_validate(&GALDR_STYLE, path, false) {
        Ok(json) => {
...
fn is_valid(_py: Python<'_>, path: &str) -> PyResult<bool> {
    Ok(read_and_validate(&GALDR_STYLE, path, false).is_ok())
}

#[pyfunction]
fn schema_name() -> &'static str {
    GALDR_STYLE.schema_name()
}

#[pymodule]
fn gate_galdr_style_input(m: &Bound<'_, PyModule>) -> PyResult<()> {
