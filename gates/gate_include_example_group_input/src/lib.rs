//! Gate: gate_include_example_group_input
//! Input gate: reads TOML from disk, validates against include-example-group schema, returns JSON.

use pyo3::prelude::*;
use pyo3::types::PyDict;

use gate_io::read_and_validate;
use schemas_embedded::INCLUDE_EXAMPLE_GROUP;

#[pyfunction]
fn validate(py: Python<'_>, path: &str) -> PyResult<PyObject> {
    match read_and_validate(&INCLUDE_EXAMPLE_GROUP, path, false) {
        Ok(json) => {
...
fn is_valid(_py: Python<'_>, path: &str) -> PyResult<bool> {
    Ok(read_and_validate(&INCLUDE_EXAMPLE_GROUP, path, false).is_ok())
}

#[pyfunction]
fn schema_name() -> &'static str {
    INCLUDE_EXAMPLE_GROUP.schema_name()
}

#[pymodule]
fn gate_include_example_group_input(m: &Bound<'_, PyModule>) -> PyResult<()> {
