...
//! Gate: permissions_resolved_input
//! TOML-to-JSON input gate with path verification.
//! Validates against permissions-resolved schema and verifies all paths exist.
...
fn gate_validate(input: &str) -> Result<String, NornirError> {
    ...
    let result = PERMISSIONS_RESOLVED.validate(&json)?;
    if !result.valid {
        return Err(error_core::SchemaError::ValidationFailed(result.message).into());
    }
    path_verify::verify_paths(PERMISSIONS_RESOLVED.schema_json(), &json)?;
    Ok(json)
}
