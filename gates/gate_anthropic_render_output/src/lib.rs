...
//! Gate: anthropic_render_output
//! JSON-to-TOML output gate with path verification.
//! Validates against anthropic-render schema and verifies all paths exist
...
fn gate_validate(input: &str) -> Result<String, NornirError> {
    let result = ANTHROPIC_RENDER.validate(input)?;
    if !result.valid {
        return Err(error_core::SchemaError::ValidationFailed(result.message).into());
    }
    path_verify::verify_paths(ANTHROPIC_RENDER.schema_json(), input)?;
