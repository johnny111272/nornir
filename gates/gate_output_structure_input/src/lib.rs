//! Gate: gate_output_structure_input
//! Input gate: reads TOML from disk, validates against output-structure schema, returns JSON.

use pyo3::prelude::*;
use pyo3::types::PyDict;

use gate_io::read_and_validate;
use schemas_embedded::OUTPUT_STRUCTURE;

#[pyfunction]
fn validate(py: Python<'_>, path: &str) -> PyResult<PyObject> {
    match read_and_validate(&OUTPUT_STRUCTURE, path, false) {
        Ok(json) => {
...
fn is_valid(_py: Python<'_>, path: &str) -> PyResult<bool> {
    Ok(read_and_validate(&OUTPUT_STRUCTURE, path, false).is_ok())
}

#[pyfunction]
fn schema_name() -> &'static str {
    OUTPUT_STRUCTURE.schema_name()
}

#[pymodule]
fn gate_output_structure_input(m: &Bound<'_, PyModule>) -> PyResult<()> {
