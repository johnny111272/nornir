//! Gate: gate_includes_merged_input
//! Input gate: reads TOML from disk, validates against includes-merged schema, returns JSON.
...
fn validate(py: Python<'_>, path: &str) -> PyResult<PyObject> {
    match read_and_validate(&INCLUDES_MERGED, path, true) {
        Ok(json) => {
...
fn is_valid(_py: Python<'_>, path: &str) -> PyResult<bool> {
    Ok(read_and_validate(&INCLUDES_MERGED, path, true).is_ok())
}

#[pyfunction]
fn schema_name() -> &'static str {
    INCLUDES_MERGED.schema_name()
}

#[pymodule]
fn gate_includes_merged_input(m: &Bound<'_, PyModule>) -> PyResult<()> {
