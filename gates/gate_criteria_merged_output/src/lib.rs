//! Gate: gate_criteria_merged_output
//! Output gate: validates JSON against criteria-merged schema, writes TOML to disk.
...
fn validate(py: Python<'_>, data: &str, path: &str) -> PyResult<PyObject> {
    match validate_and_write(&CRITERIA_MERGED, data, path, true) {
        Ok(()) => {
...
fn is_valid(_py: Python<'_>, data: &str) -> PyResult<bool> {
    Ok(CRITERIA_MERGED.is_valid(data).unwrap_or(false))
}
...
#[pymodule]
fn gate_criteria_merged_output(m: &Bound<'_, PyModule>) -> PyResult<()> {
